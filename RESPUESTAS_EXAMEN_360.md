# Respuestas Correctas - Examen 360
...
- D) Big Bang Deployment (no es una estrategia recomendada)
...
- C) Ejecutar todos los tests en secuencia (esto aumenta el tiempo)
...
- C) GitHub Actions ✓
- D) CircleCI ✓
- E) Docker (es una herramienta de containerización, no específicamente CI/CD)
...
- D) Rollback automático en caso de fallos (generalmente es manual o semi-automático)
...
- D) Performance tests en producción (generalmente en staging)
...
- E) Secrets en variables de entorno sin encriptar (inseguro)
...
- E) Reducción del tiempo de build (no necesariamente)
...
- C) CloudFormation ✓
- D) Pulumi ✓
- E) Docker Compose (no es específicamente para IaC)
...
- E) Re-ejecución automática en fallos intermitentes (puede ser problemático)
...
- B) Scripted Pipeline ✓
- C) YAML Pipeline (no es estándar en Jenkins)
- D) JSON Pipeline (no es estándar)
- E) Solo Groovy (ambos usan Groovy)
...
- E) Hardcoding de valores en el pipeline (mala práctica)
...
- D) La estrategia de comunicación de cambios (no es parte de deployment strategy)
...
- C) OWASP ZAP ✓
- D) Trivy ✓
- E) ESLint (es para linting, no específicamente seguridad)
...
- E) Branches de larga duración (no es parte de trunk-based)
...
- E) Jobs que dependen de estado externo no controlado (reduce confiabilidad)
...
- B) Mutation ✓
- C) Subscription ✓
- D) Operation (es el término genérico)
- E) Resolver (no es una operación)
...
- E) Solo afecta a mutations (afecta a queries también)
...
- D) Validación de tipos en runtime (es parte pero no la definición)
- E) Solo una característica de desarrollo (también en producción)
...
- C) Mutations ✓
- D) Subscriptions ✓
- E) Resolvers (se definen en código, no en schema)
...
- E) Solo relevante para mutations (también para queries)
...
- B) GraphQL.js ✓
- C) Hasura ✓
- D) Relay (es un framework de cliente, no servidor)
- E) Express GraphQL ✓
...
- E) Solo para mutations (también para queries)
...
- E) Cambios breaking frecuentes (mala práctica)
...
- E) GraphQL siempre es mejor que REST (depende del caso)
...
- E) Availability (Disponibilidad) (no es parte de ACID)
...
- C) REPEATABLE READ ✓
- D) SERIALIZABLE ✓
- E) READ ONLY (no es un nivel de aislamiento)
...
- E) No se puede prevenir (sí se puede prevenir)
...
- E) Siempre es una mala práctica (no siempre)
...
- E) Siempre es ineficiente (no siempre)
...
- E) Solo para debugging (también para optimización)
...
- E) No se puede prevenir (sí se puede)
...
- E) Solo para bases de datos NoSQL (también para SQL)
...
- E) Siempre mejora performance (no siempre)
...
- E) Solo para PostgreSQL (también en otros DBMS)
...
- E) Solo para bases de datos NoSQL (también en SQL)
...
- E) Solo para desarrollo (también en producción)

---

## Consejos para el Examen

1. **Lee cuidadosamente cada pregunta** - Algunas tienen múltiples respuestas correctas
2. **No asumas que solo hay una respuesta correcta** - El examen permite múltiples selecciones
3. **Si no estás seguro, marca todas las opciones que consideres plausibles** - Es mejor marcar de más que de menos
4. **Revisa conceptos que no dominas** - Usa el RESUMEN_ESTUDIO.md para repasar
5. **Practica con tiempo** - No te apresures, lee cada opción cuidadosamente
